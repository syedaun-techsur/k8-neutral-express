...
| **Triage Remainder** | Scans remaining notes; identifies 3 stale ones for deletion | Note List `/` (F0) | "Which of these can I get rid of? I'll delete the ones from last month." | Efficient, decisive | No creation date visible on note cards — can't tell which notes are old | Creation date displayed as a subtle secondary label on each note card |
